...
import { RecentPlayArguments, RecentScore } from "../osu/recent/recentHandler";

export function calcRetries(plays: OsuScore[], mapid: number, mods: { acronym: string; }[]) {
    const modAcronyms = mods.map(mod => mod.acronym);
    const trys = plays.filter(r => 
        r.beatmap.id === mapid && arrayEquals(r.mods.map(m => m.acronym), modAcronyms)
    );
    return trys.length;
}
